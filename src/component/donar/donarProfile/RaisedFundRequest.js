...
import { useDeleteRaiseFundFromDonarMutation, useGetRaiseFundAssignToDonarQuery } from "../../../services/SignUpApi";

const RaisedFundRequest = (props) => {
  const ID = props.ID;
  ...
  const { data, isSuccess } = useGetRaiseFundAssignToDonarQuery(ID);
  //   console.log(data);
  ...
  useEffect(() => {
    const filteredData = raiseFundStudent?.filter((entry) => {
      ...
      const currentCourse = entry.currentCourse || "";
      return (
  ...
  const [deleteRaiseFundFromDonar] = useDeleteRaiseFundFromDonarMutation()

  const handleClick = async (raiseFundId) => {
    const formData = { raiseFundId, donarId:ID };
    console.log(formData);
    const res = await deleteRaiseFundFromDonar(formData);
...
export default RaisedFundRequest;
