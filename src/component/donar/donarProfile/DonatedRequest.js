...
import { useGetDonationHistoryQuery } from "../../../services/SignUpApi";

const DonatedRequest = (props) => {
  const ID=props.ID;
  ...
  const { data,isSuccess} =useGetDonationHistoryQuery(ID);
  useEffect(() => {
                ...
                {getCurrentPageEntries().length === 0 ? (
                  ...
                  >
                    ...
                    <tbody className="bg-white divide-y divide-slate-100 dark:bg-slate-800 dark:divide-slate-700">
                      {getCurrentPageEntries().filter(data => data.status === "SUCCESS")
                      .map((data) => {
                        const id = data.id;
                        ...
                        return (
                          <tr key={id}>
                            ...
                            <td className="table-td">  {data.panNumber !== null ? data.panNumber : "-"}</td>
                            <td className="table-td">
